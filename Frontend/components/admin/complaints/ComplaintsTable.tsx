...
export default function ComplaintsTable({
  ...
  const handleDeleteComplaint = async (complaint: Complaint) => {
    if (!confirm(`Are you sure you want to delete complaint "${complaint.title}" (ID: ${complaint.id})?`)) {
      return
    }
    ...
    try {
      ...
      setTableComplaints(prev => prev.filter(c => c.comp_id !== complaint.comp_id))
  ...
  const filteredComplaints = tableComplaints.filter((complaint) => {
    ...
    const matchesSearch = searchQuery === '' || 
      ...
      (complaint.comp_id && complaint.comp_id.toString().toLowerCase().includes(searchQuery.toLowerCase())) ||
